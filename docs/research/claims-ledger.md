...
| 9 | Paired benchmark with ≥30 replicates, per-trajectory CIs, fault injection. | **P** | protocol only (`paired-experiment-protocol.md`); committed `results/v1.1/` are single-replicate and now **stale** vs the hardened solver | — | any headline benchmark superiority claim |
