...
class RobotController(Node):
    def __init__(self) -> None:
        ...
        self.ee_pub = self.create_publisher(PoseStamped, str(self.get_parameter("ee_pose_topic").value), 10)
        self.js_pub = None
        ...
        self.create_subscription(PoseStamped, str(self.get_parameter("input_topic").value), self._pose_cb, 10)
        self.create_subscription(Bool, "/teleop/cancel_trajectory", self._cancel_cb, 10)
        self.get_logger().info(f"robot_controller mode={self.mode} backend=simulation (no physical hardware)")
    ...
    def _pose_cb(self, msg: PoseStamped) -> None:
        ...
        alpha = 0.55
        blended = [(1 - alpha) * a + alpha * b for a, b in zip(self._last_joints, q)]
