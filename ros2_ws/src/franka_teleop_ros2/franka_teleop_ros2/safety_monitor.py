...
class SafetyMonitorNode(Node):
    def __init__(self) -> None:
        ...
        if control_mode == ControlMode.REAL_ROBOT:
            self.get_logger().error("REAL ROBOT MODE — software safeguards do not replace Franka E-stop.")

        self.pub = self.create_publisher(PoseStamped, str(self.get_parameter("output_topic").value), 10)
        ...
        self.get_logger().info(
            f"safety_monitor mode={self.monitor.control_mode.value} " f"allow_real={allow_real}"
        )
