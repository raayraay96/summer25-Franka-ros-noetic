...
class HumanToRobotMapper(Node):
    def __init__(self) -> None:
        ...
        self.pub = self.create_publisher(PoseStamped, str(self.get_parameter("target_topic").value), 10)
        self.rej = self.create_publisher(String, str(self.get_parameter("rejected_topic").value), 10)
        self.get_logger().info(f"human_to_robot_mapper mode={self.mode} depth_for_control={self.use_metric}")
        self.get_logger().warn("Camera extrinsics/intrinsics are EXAMPLE values unless calibrated.")
    ...
    def _cb(self, msg: String) -> None:
        ...
        shoulder_xy = (float(shoulder["x"]), float(shoulder["y"])) if shoulder else None
        try:
